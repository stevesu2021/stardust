    ...
    <view class="list">
      <view class="item" v-for="item in list" :key="item.id">
        ...
        <view class="item-content">
          ...
          <text class="target" v-if="item.targetName">致: {{ item.targetName }}</text>
        </view>
        <view class="item-footer">
          <text class="count">🙏 {{ item.prayerCount }}</text>
          <text class="action" @click="increment(item.id)">+1</text>
        </view>
      </view>
    </view>
...
function create() {
  uni.navigateTo({ url: '/pages/prayer/create' })
}
