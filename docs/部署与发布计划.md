# 星契集 - 部署与发布计划

## 📋 部署架构总览

### 整体架构图
```
┌─────────────────────────────────────────────────────────┐
│                     用户端 (多端)                        │
│  ┌─────────┐  ┌──────────┐  ┌──────────┐  ┌─────────┐ │
│  │  H5网站 │  │微信小程序│  │抖音小程序│  │ App端   │ │
│  └────┬────┘  └────┬─────┘  └────┬─────┘  └────┬────┘ │
│       │            │             │             │        │
└───────┼────────────┼─────────────┼─────────────┼────────┘
        │            │             │             │
        └────────────┴─────────────┴─────────────┴────────┐
                                                           │
┌─────────────────────────────────────────────────────────┐
│                  CDN / 负载均衡层                        │
│  ┌──────────────────────────────────────────────────┐   │
│  │  Nginx (反向代理)                                │   │
│  │  - 静态资源缓存                                  │   │
│  │  - API 路由分发                                  │   │
│  │  - SSL 终端                                      │   │
│  └──────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────┐
│                  应用服务层                             │
│  ┌──────────────────────────────────────────────────┐   │
│  │  Nest.js API 服务 (多实例)                       │   │
│  │  - 认证服务                                      │   │
│  │  - 业务逻辑                                      │   │
│  │  - WebSocket (实时聊天)                          │   │
│  └──────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────┐
│                  数据层                                 │
│  ┌──────────┐  ┌──────────┐  ┌──────────────────┐      │
│  │ PostgreSQL│  │  Redis   │  │  对象存储 OSS    │      │
│  │ (主从)   │  │ (缓存)   │  │ (图片/文件)      │      │
│  └──────────┘  └──────────┘  └──────────────────┘      │
└─────────────────────────────────────────────────────────┘
```

---

## 🖥️ 环境配置

### 一、开发环境

#### 1.1 本地开发环境
```bash
# 前端开发
cd frontend
npm install
npm run dev:h5          # H5 开发
npm run dev:mp-weixin   # 微信小程序开发
npm run dev:mp-toutiao  # 抖音小程序开发
npm run dev:app         # App 开发

# 后端开发
cd backend
npm install
npm run start:dev       # 开发模式启动

# 数据库
docker-compose up -d db redis
cd backend
npx prisma migrate dev
npx prisma db seed
```

#### 1.2 开发环境配置
```env
# backend/.env.development
PORT=3000
DATABASE_URL="postgresql://user:pass@localhost:5432/stardust_dev"
REDIS_URL="redis://localhost:6379"
JWT_SECRET="dev-secret-key"
JWT_EXPIRES_IN="7d"

# 前端代理配置 (vite.config.ts)
proxy: {
  '/api': {
    target: 'http://localhost:3000',
    changeOrigin: true
  }
}
```

### 二、测试环境

#### 2.1 测试环境配置
```yaml
# docker-compose.test.yml
version: '3.8'

services:
  backend:
    build: ./backend
    environment:
      - NODE_ENV=test
      - DATABASE_URL=postgresql://test:pass@test-db:5432/stardust_test
      - REDIS_URL=redis://test-redis:6379
      - JWT_SECRET=test-secret-key
    ports:
      - "3001:3000"
    depends_on:
      - test-db
      - test-redis

  test-db:
    image: postgres:15
    environment:
      POSTGRES_DB: stardust_test
      POSTGRES_USER: test
      POSTGRES_PASSWORD: pass
    ports:
      - "5433:5432"

  test-redis:
    image: redis:7-alpine
    ports:
      - "6380:6379"

  frontend:
    build: ./frontend
    ports:
      - "8081:80"
    environment:
      - API_BASE_URL=http://backend:3000
```

#### 2.2 自动化测试
```bash
# 后端测试
cd backend
npm run test          # 单元测试
npm run test:e2e      # E2E测试
npm run test:cov      # 代码覆盖率

# 前端测试 (待配置)
cd frontend
npm run test:unit
npm run test:e2e
```

### 三、生产环境

#### 3.1 生产环境配置
```env
# backend/.env.production
PORT=3000
DATABASE_URL="postgresql://user:pass@prod-db:5432/stardust"
REDIS_URL="redis://prod-redis:6379"
JWT_SECRET="prod-secret-key-from-vault"
JWT_EXPIRES_IN="7d"

# CDN 配置
CDN_URL="https://cdn.stardust.app"
OSS_ACCESS_KEY="your-oss-key"
OSS_SECRET="your-oss-secret"

# 监控配置
SENTRY_DSN="https://xxx.ingest.sentry.io/xxx"
```

#### 3.2 生产环境安全
```bash
# 生成强密码
openssl rand -base64 32

# 密钥管理 (使用 Vault 或云服务商 KMS)
# AWS Secrets Manager / 阿里云 KMS / 腾讯云 Secrets Manager
```

---

## 🐳 Docker 部署

### 一、前端 Docker 化

#### 1.1 H5 Dockerfile
```dockerfile
# frontend/Dockerfile.h5

# 构建阶段
FROM node:18-alpine AS builder

WORKDIR /app

# 复制依赖
COPY package*.json ./
RUN npm ci --only=production

# 复制源码
COPY . .

# 构建 H5
RUN npm run build:h5

# 运行阶段
FROM nginx:alpine

# 复制构建产物
COPY --from=builder /app/dist/h5 /usr/share/nginx/html

# 复制 Nginx 配置
COPY nginx.conf /etc/nginx/nginx.conf

# 设置时区
RUN apk add --no-cache tzdata \
    && cp /usr/share/zoneinfo/Asia/Shanghai /etc/localtime \
    && echo "Asia/Shanghai" > /etc/timezone

EXPOSE 80

CMD ["nginx", "-g", "daemon off;"]
```

#### 1.2 Nginx 配置
```nginx
# frontend/nginx.conf

events {
    worker_connections 1024;
}

http {
    include /etc/nginx/mime.types;
    default_type application/octet-stream;

    # Gzip 压缩
    gzip on;
    gzip_vary on;
    gzip_min_length 1024;
    gzip_types
        text/plain
        text/css
        application/json
        application/javascript
        text/xml
        application/xml
        application/xml+rss
        text/javascript;

    # 缓存策略
    map $uri $cache_control {
        ~*\.(js|css)$ "public, max-age=31536000, immutable";
        ~*\.(png|jpg|jpeg|gif|ico|svg)$ "public, max-age=31536000, immutable";
        default "no-cache";
    }

    server {
        listen 80;
        server_name stardust.app www.stardust.app;

        # 安全头
        add_header X-Frame-Options "SAMEORIGIN" always;
        add_header X-Content-Type-Options "nosniff" always;
        add_header X-XSS-Protection "1; mode=block" always;

        # SPA 路由支持
        location / {
            root /usr/share/nginx/html;
            try_files $uri $uri/ /index.html;
            add_header Cache-Control $cache_control;
        }

        # API 代理
        location /api/ {
            proxy_pass http://backend:3000/;
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            proxy_set_header X-Forwarded-Proto $scheme;

            # WebSocket 支持
            proxy_http_version 1.1;
            proxy_set_header Upgrade $http_upgrade;
            proxy_set_header Connection "upgrade";
        }

        # 健康检查
        location /health {
            access_log off;
            return 200 "healthy\n";
            add_header Content-Type text/plain;
        }
    }
}
```

#### 1.3 小程序构建
```bash
# 微信小程序
npm run build:mp-weixin
# 生成目录: dist/build/mp-weixin

# 抖音小程序
npm run build:mp-toutiao
# 生成目录: dist/build/mp-toutiao

# 上传至各平台后台
```

### 二、后端 Docker 化

#### 2.1 后端 Dockerfile
```dockerfile
# backend/Dockerfile

# 构建阶段
FROM node:18-alpine AS builder

WORKDIR /app

# 复制 package.json
COPY package*.json ./

# 安装所有依赖 (包括 devDependencies)
RUN npm ci

# 复制源码
COPY . .

# 构建
RUN npm run build

# 运行阶段
FROM node:18-alpine

WORKDIR /app

# 复制 package.json
COPY package*.json ./

# 只安装生产依赖
RUN npm ci --only=production && npm cache clean --force

# 复制构建产物
COPY --from=builder /app/dist ./dist

# 复制 Prisma 客户端
COPY --from=builder /app/node_modules/.prisma ./node_modules/.prisma

# 设置时区
RUN apk add --no-cache tzdata \
    && cp /usr/share/zoneinfo/Asia/Shanghai /etc/localtime \
    && echo "Asia/Shanghai" > /etc/timezone

# 创建非 root 用户
RUN addgroup -g 1001 -S nodejs \
    && adduser -S nestjs -u 1001

# 更改目录权限
RUN chown -R nestjs:nodejs /app

USER nestjs

EXPOSE 3000

CMD ["node", "dist/main"]
```

#### 2.2 Docker Compose 生产配置
```yaml
# docker-compose.prod.yml

version: '3.8'

services:
  # Nginx 反向代理
  nginx:
    image: nginx:alpine
    ports:
      - "80:80"
      - "443:443"
    volumes:
      - ./nginx.conf:/etc/nginx/nginx.conf
      - ./ssl:/etc/nginx/ssl
      - frontend-dist:/usr/share/nginx/html
    depends_on:
      - backend
    restart: unless-stopped
    healthcheck:
      test: ["CMD", "curl", "-f", "http://localhost/health"]
      interval: 30s
      timeout: 10s
      retries: 3

  # 后端 API (多实例)
  backend:
    build: ./backend
    environment:
      - NODE_ENV=production
      - DATABASE_URL=postgresql://user:${DB_PASSWORD}@db:5432/stardust
      - REDIS_URL=redis://redis:6379
      - JWT_SECRET=${JWT_SECRET}
    deploy:
      replicas: 3
      resources:
        limits:
          cpus: '1'
          memory: 512M
        reservations:
          cpus: '0.5'
          memory: 256M
    depends_on:
      - db
      - redis
    restart: unless-stopped
    healthcheck:
      test: ["CMD", "curl", "-f", "http://localhost:3000/health"]
      interval: 30s
      timeout: 10s
      retries: 3

  # PostgreSQL 主库
  db:
    image: postgres:15
    environment:
      POSTGRES_DB: stardust
      POSTGRES_USER: user
      POSTGRES_PASSWORD: ${DB_PASSWORD}
    volumes:
      - postgres_data:/var/lib/postgresql/data
      - ./init.sql:/docker-entrypoint-initdb.d/init.sql
    ports:
      - "5432:5432"
    restart: unless-stopped
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U user"]
      interval: 10s
      timeout: 5s
      retries: 5

  # PostgreSQL 从库 (读写分离)
  db-replica:
    image: postgres:15
    environment:
      POSTGRES_DB: stardust
      POSTGRES_USER: user
      POSTGRES_PASSWORD: ${DB_PASSWORD}
    volumes:
      - postgres_replica_data:/var/lib/postgresql/data
    depends_on:
      - db
    restart: unless-stopped

  # Redis 缓存
  redis:
    image: redis:7-alpine
    command: redis-server --appendonly yes --requirepass ${REDIS_PASSWORD}
    volumes:
      - redis_data:/data
    ports:
      - "6379:6379"
    restart: unless-stopped
    healthcheck:
      test: ["CMD", "redis-cli", "ping"]
      interval: 10s
      timeout: 3s
      retries: 5

  # 监控 (可选)
  prometheus:
    image: prom/prometheus
    ports:
      - "9090:9090"
    volumes:
      - ./prometheus.yml:/etc/prometheus/prometheus.yml
    restart: unless-stopped

  grafana:
    image: grafana/grafana
    ports:
      - "3000:3000"
    environment:
      - GF_SECURITY_ADMIN_PASSWORD=${GRAFANA_PASSWORD}
    volumes:
      - grafana_data:/var/lib/grafana
    restart: unless-stopped

volumes:
  postgres_data:
  postgres_replica_data:
  redis_data:
  grafana_data:
  frontend-dist:
```

### 三、CI/CD 流程

#### 3.1 GitHub Actions 配置
```yaml
# .github/workflows/deploy.yml

name: Deploy to Production

on:
  push:
    branches: [ main ]
  pull_request:
    branches: [ main ]

jobs:
  test:
    runs-on: ubuntu-latest

    services:
      postgres:
        image: postgres:15
        env:
          POSTGRES_PASSWORD: postgres
          POSTGRES_DB: test
        ports:
          - 5432:5432
        options: --health-cmd pg_isready --health-interval 10s --health-timeout 5s --health-retries 5

      redis:
        image: redis:7-alpine
        ports:
          - 6379:6379
        options: --health-cmd "redis-cli ping" --health-interval 10s --health-timeout 5s --health-retries 5

    steps:
      - uses: actions/checkout@v3

      # 后端测试
      - name: Setup Node.js
        uses: actions/setup-node@v3
        with:
          node-version: '18'
          cache: 'npm'
          cache-dependency-path: backend/package-lock.json

      - name: Install Backend Dependencies
        working-directory: ./backend
        run: npm ci

      - name: Run Backend Tests
        working-directory: ./backend
        env:
          DATABASE_URL: postgresql://postgres:postgres@localhost:5432/test
          REDIS_URL: redis://localhost:6379
          JWT_SECRET: test-secret
        run: npm run test:cov

      - name: Build Backend
        working-directory: ./backend
        run: npm run build

      # 前端构建
      - name: Setup Frontend
        working-directory: ./frontend
        run: npm ci

      - name: Build Frontend
        working-directory: ./frontend
        run: npm run build:h5

      # Docker 构建
      - name: Login to Docker Hub
        uses: docker/login-action@v2
        with:
          username: ${{ secrets.DOCKER_USERNAME }}
          password: ${{ secrets.DOCKER_PASSWORD }}

      - name: Build and Push Backend
        uses: docker/build-push-action@v4
        with:
          context: ./backend
          push: true
          tags: |
            yourusername/stardust-backend:latest
            yourusername/stardust-backend:${{ github.sha }}

      - name: Build and Push Frontend
        uses: docker/build-push-action@v4
        with:
          context: ./frontend
          file: ./frontend/Dockerfile.h5
          push: true
          tags: |
            yourusername/stardust-frontend:latest
            yourusername/stardust-frontend:${{ github.sha }}

  deploy:
    needs: test
    runs-on: ubuntu-latest
    if: github.ref == 'refs/heads/main'

    steps:
      - name: Deploy to Production
        uses: appleboy/ssh-action@v1.0.0
        with:
          host: ${{ secrets.PROD_HOST }}
          username: ${{ secrets.PROD_USER }}
          key: ${{ secrets.SSH_PRIVATE_KEY }}
          script: |
            cd /opt/stardust
            docker-compose -f docker-compose.prod.yml pull
            docker-compose -f docker-compose.prod.yml up -d --remove-orphans
            docker system prune -f
```

---

## ☁️ 云服务商部署方案

### 一、阿里云部署方案

#### 1.1 资源配置
```
ECS 实例:
  - 配置: 2核4G (生产环境建议 4核8G)
  - 系统: Ubuntu 20.04 LTS
  - 带宽: 5Mbps (根据流量调整)
  - 价格: ~¥200/月 (按量付费)

RDS PostgreSQL:
  - 配置: 2核4G
  - 存储: 100GB (自动扩容)
  - 版本: PostgreSQL 15
  - 价格: ~¥400/月

Redis:
  - 配置: 1GB
  - 版本: Redis 7
  - 价格: ~¥100/月

OSS 对象存储:
  - 存储: 50GB
  - 流量: 100GB/月
  - 价格: ~¥50/月

CDN:
  - 流量: 100GB/月
  - 价格: ~¥20/月

总计: ~¥770/月
```

#### 1.2 部署步骤
```bash
# 1. 购买并配置 ECS
# - 开放端口: 80, 443, 3000 (可选)
# - 配置安全组规则

# 2. 连接服务器
ssh root@your-ecs-ip

# 3. 安装基础软件
apt update
apt install -y docker.io docker-compose git

# 4. 克隆代码
git clone https://github.com/yourusername/stardust.git /opt/stardust

# 5. 配置环境变量
cd /opt/stardust
cp .env.example .env.prod
# 编辑 .env.prod 填入真实配置

# 6. 部署
docker-compose -f docker-compose.prod.yml up -d

# 7. 配置 SSL (使用 Let's Encrypt)
docker run -it --rm \
  -v /opt/stardust/ssl:/etc/letsencrypt \
  certbot/certbot certonly --nginx -d stardust.app -d www.stardust.app
```

#### 1.3 域名与 SSL
```
域名购买: 阿里云域名
  - stardust.app (¥50/年)
  - 星契集.app (¥50/年)

SSL 证书:
  - Let's Encrypt (免费)
  - 自动续期脚本

CDN 配置:
  - 开启 Gzip
  - 配置缓存策略
  - 开启 HTTPS
```

### 二、腾讯云部署方案

#### 2.1 资源配置
```
CVM 实例:
  - 配置: 2核4G
  - 系统: Ubuntu 20.04
  - 带宽: 5Mbps
  - 价格: ~¥180/月

PostgreSQL:
  - 配置: 2核4G
  - 存储: 100GB
  - 价格: ~¥380/月

Redis:
  - 配置: 1GB
  - 价格: ~¥90/月

COS 对象存储:
  - 存储: 50GB
  - 价格: ~¥40/月

CDN:
  - 流量: 100GB/月
  - 价格: ~¥20/月

总计: ~¥710/月
```

#### 2.2 部署脚本
```bash
#!/bin/bash
# deploy.sh

set -e

echo "🚀 开始部署 StarDust..."

# 拉取最新代码
cd /opt/stardust
git pull origin main

# 构建前端
cd frontend
docker build -t stardust-frontend:latest -f Dockerfile.h5 .

# 构建后端
cd ../backend
docker build -t stardust-backend:latest .

# 停止旧容器
cd ..
docker-compose -f docker-compose.prod.yml down

# 启动新容器
docker-compose -f docker-compose.prod.yml up -d

# 清理旧镜像
docker image prune -f

echo "✅ 部署完成！"
```

### 三、AWS 部署方案 (国际版)

#### 3.1 资源配置
```
EC2 (应用服务器):
  - 实例: t3.medium (2核4G)
  - 系统: Amazon Linux 2
  - 价格: ~$30/月

RDS (数据库):
  - 实例: db.t3.micro
  - 引擎: PostgreSQL 15
  - 价格: ~$15/月

ElastiCache (Redis):
  - 实例: cache.t3.micro
  - 价格: ~$10/月)

S3 (对象存储):
  - 存储: 50GB
  - 流量: 100GB
  - 价格: ~$5/月

CloudFront (CDN):
  - 流量: 100GB
  - 价格: ~$10/月

总计: ~$70/月
```

#### 3.2 部署流程
```bash
# 1. 配置 AWS CLI
aws configure

# 2. 创建 ECS 集群
aws ecs create-cluster --cluster-name stardust-cluster

# 3. 推送镜像到 ECR
aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin <account-id>.dkr.ecr.us-east-1.amazonaws.com

docker tag stardust-backend:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/stardust-backend:latest
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/stardust-backend:latest

# 4. 部署服务
aws ecs create-service --cluster stardust-cluster --service-name backend-service --task-definition backend-task
```

---

## 📱 各平台发布流程

### 一、H5 网站发布

#### 1.1 域名与 CDN 配置
```
域名: stardust.app
SSL: Let's Encrypt (自动续期)

CDN 配置:
  - 阿里云 CDN / Cloudflare
  - 缓存策略:
    * HTML: no-cache
    * JS/CSS: 1年
    * 图片: 1周
  - 开启 Gzip/Brotli
  - 开启 HTTP/2
  - 配置安全防护 (WAF)

DNS 配置:
  A 记录: stardust.app -> 服务器 IP
  CNAME: www -> stardust.app
```

#### 1.2 部署检查清单
```bash
□ 域名解析生效
□ SSL 证书安装
□ CDN 配置完成
□ 首页加载测试 (< 2s)
□ API 接口测试
□ 移动端适配测试
□ 微信分享测试
□ 百度收录提交
□ Google Analytics 配置
□ 错误监控配置 (Sentry)
```

### 二、微信小程序发布

#### 2.1 开发准备
```
小程序账号:
  - 注册小程序账号 (https://mp.weixin.qq.com)
  - 企业认证 (¥300/年)
  - 开通支付功能 (需企业资质)

开发配置:
  - AppID: wxxxxxxxxxxxxxxxx
  - 服务器域名配置:
    * request合法域名: https://api.stardust.app
    * socket合法域名: wss://api.stardust.app
    * uploadFile域名: https://api.stardust.app
    * downloadFile域名: https://api.stardust.app

隐私协议:
  - 配置用户隐私保护指引
  - 声明收集的用户信息
```

#### 2.2 构建与上传
```bash
# 1. 构建小程序
cd frontend
npm run build:mp-weixin

# 2. 使用微信开发者工具
# - 打开 dist/build/mp-weixin 目录
# - 预览测试
# - 上传代码

# 3. 提交审核
# - 填写版本描述
# - 选择体验版
# - 提交审核

# 4. 审核通过后发布
```

#### 2.3 审核注意事项
```
内容合规:
  □ 避免封建迷信词汇
  □ 使用"文化"、"娱乐"、"参考"等表述
  □ 添加免责声明: "仅供娱乐参考"

功能合规:
  □ 用户协议和隐私政策
  □ 支付功能合规
  □ 用户投诉处理机制

界面规范:
  □ 不使用官方 UI 样式
  □ 不诱导分享
  □ 不滥用通知
```

#### 2.4 发布检查清单
```bash
□ 代码审核通过
□ 服务器域名配置
□ 隐私协议配置
□ 支付功能测试
□ 用户登录测试
□ 分享功能测试
□ 消息推送测试
□ 数据上报配置
□ 错误监控配置
□ 版本回滚方案
```

### 三、抖音小程序发布

#### 3.1 开发准备
```
小程序账号:
  - 注册抖音小程序账号 (https://microapp.bytedance.com)
  - 企业认证
  - 开通支付功能

开发配置:
  - AppID: ttxxxxxxxxxxxxxxx
  - 服务器域名:
    * API域名: https://api.stardust.app
    * WebSocket域名: wss://api.stardust.app
```

#### 3.2 构建与上传
```bash
# 1. 构建抖音小程序
cd frontend
npm run build:mp-toutiao

# 2. 使用字节开发者工具
# - 打开 dist/build/mp-toutiao
# - 预览测试
# - 上传代码

# 3. 提交审核
# - 填写版本信息
# - 选择发布范围
# - 提交审核
```

#### 3.3 抖音特性适配
```
短视频联动:
  - 生成星座短视频
  - 一键发布到抖音
  - 挑战赛活动

直播功能:
  - 命理师直播
  - 情感咨询直播
  - 互动打赏

算法推荐:
  - 优化内容标签
  - 提高完播率
  - 增加互动
```

### 四、App 发布

#### 4.1 iOS App Store 发布

**准备工作**
```
Apple 开发者账号:
  - 注册: https://developer.apple.com
  - 费用: $99/年
  - 企业认证

证书配置:
  - App ID: com.stardust.app
  - 证书: Distribution Certificate
  - 描述文件: Provisioning Profile
```

**构建与打包**
```bash
# 1. 配置 manifest.json
# - App 图标 (1024x1024)
# - 启动图
# - 应用名称
# - 版本号

# 2. 使用 HBuilderX 或 CLI
cd frontend
npm run build:app

# 3. 生成 IPA
# - 使用 HBuilderX 打包
# - 选择 "发行" -> "App Store 包"
# - 填写证书信息

# 4. 上传到 App Store Connect
# - 使用 Transporter 工具
# - 填写元数据
```

**App Store 审核**
```
审核信息:
  - 应用名称: 星契集
  - 副标题: 星座占卜与情感社交
  - 关键词: 星座,占卜,情感,社交,运势
  - 描述: 详细功能介绍
  - 截图: 5.5英寸和6.5英寸

隐私政策:
  - 需要提供隐私政策网址
  - 详细说明数据收集用途
  - 用户数据删除说明

审核注意事项:
  □ 不能包含宗教迷信内容
  □ 需要明确的免责声明
  □ 用户数据安全
  □ 支付功能合规
  □ 无崩溃和Bug
```

#### 4.2 Android 多渠道发布

**Google Play (国际)**
```bash
# 1. Google Play 开发者账号
# - 注册: https://play.google.com/console
# - 费用: $25 一次性
# - 企业认证

# 2. 构建 AAB (Android App Bundle)
cd frontend
npm run build:app

# 3. 签名
# - 生成签名密钥
# - 配置 build.json

# 4. 上传到 Google Play
# - 填写商店信息
# - 内容分级问卷
# - 提交审核
```

**国内应用商店**
```
应用商店:
  - 应用宝 (腾讯)
  - 华为应用市场
  - 小米应用商店
  - OPPO 软件商店
  - vivo 应用商店
  - 360 手机助手

各商店要求:
  □ 企业营业执照
  □ 软件著作权
  □ ICP 备案
  □ 隐私政策
  □ 应用说明

发布流程:
  1. 注册各商店开发者账号
  2. 提交应用审核
  3. 等待审核 (1-7天)
  4. 上线发布
```

---

## 🛠️ 运维监控

### 一、监控体系

#### 1.1 系统监控 (Prometheus + Grafana)
```yaml
# prometheus.yml
global:
  scrape_interval: 15s

scrape_configs:
  - job_name: 'backend'
    static_configs:
      - targets: ['backend:3000']
    metrics_path: '/metrics'

  - job_name: 'postgres'
    static_configs:
      - targets: ['postgres:5432']

  - job_name: 'redis'
    static_configs:
      - targets: ['redis:6379']

  - job_name: 'nginx'
    static_configs:
      - targets: ['nginx:80']
```

**监控指标**
```
系统指标:
  - CPU 使用率 (>80% 告警)
  - 内存使用率 (>80% 告警)
  - 磁盘使用率 (>85% 告警)
  - 网络流量

应用指标:
  - API 响应时间 (<500ms)
  - 错误率 (<1%)
  - 并发连接数
  - 请求吞吐量

数据库指标:
  - 连接数
  - 查询性能
  - 缓存命中率
  - 主从延迟

Redis 指标:
  - 内存使用
  - 命中率
  - 键数量
  - 慢查询
```

#### 1.2 应用监控 (Sentry)
```javascript
// backend/src/main.ts
import * as Sentry from '@sentry/node';

Sentry.init({
  dsn: process.env.SENTRY_DSN,
  environment: process.env.NODE_ENV,
  tracesSampleRate: 1.0,
});

// 前端监控
// frontend/src/main.ts
import * as Sentry from '@sentry/vue';

Sentry.init({
  app,
  dsn: process.env.SENTRY_DSN,
  integrations: [
    new Sentry.BrowserTracing(),
    new Sentry.Replay()
  ],
  tracesSampleRate: 1.0,
  replaysSessionSampleRate: 0.1,
  replaysOnErrorSampleRate: 1.0,
});
```

#### 1.3 日志系统
```bash
# 使用 ELK Stack 或 Loki

# 日志收集配置
docker run -d \
  --name logstash \
  -p 5000:5000 \
  -v /opt/stardust/logstash.conf:/usr/share/logstash/pipeline/logstash.conf \
  docker.elastic.co/logstash/logstash:8.0.0
```

**日志级别**
```
ERROR: 系统错误，立即处理
WARN: 警告信息，需要关注
INFO: 重要操作记录
DEBUG: 调试信息
```

### 二、告警配置

#### 2.1 告警规则
```yaml
# alert-rules.yml

groups:
  - name: backend
    rules:
      - alert: HighErrorRate
        expr: rate(http_requests_total{status=~"5.."}[5m]) > 0.05
        for: 5m
        labels:
          severity: critical
        annotations:
          summary: "高错误率"
          description: "错误率超过 5%"

      - alert: SlowResponse
        expr: histogram_quantile(0.95, rate(http_request_duration_seconds_bucket[5m])) > 1
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "响应缓慢"
          description: "95% 请求响应时间超过 1秒"

  - name: database
    rules:
      - alert: DatabaseDown
        expr: up{job="postgres"} == 0
        for: 1m
        labels:
          severity: critical
        annotations:
          summary: "数据库宕机"
```

#### 2.2 告警渠道
```
告警方式:
  - 钉钉/企业微信
  - 邮件
  - 短信 (紧急)
  - 电话 (严重)

告警分级:
  - P0 (紧急): 电话+短信+邮件
  - P1 (严重): 短信+邮件
  - P2 (一般): 邮件+钉钉
```

### 三、备份策略

#### 3.1 数据库备份
```bash
#!/bin/bash
# backup-db.sh

DATE=$(date +%Y%m%d_%H%M%S)
BACKUP_DIR="/backup/postgresql"
FILENAME="stardust_${DATE}.sql"

# 全量备份
pg_dump -h db -U user -d stardust > ${BACKUP_DIR}/${FILENAME}

# 压缩
gzip ${BACKUP_DIR}/${FILENAME}

# 上传到 OSS
ossutil cp ${BACKUP_DIR}/${FILENAME}.gz oss://stardust-backups/db/

# 保留最近7天
find ${BACKUP_DIR} -name "*.sql.gz" -mtime +7 -delete

# 发送通知
echo "数据库备份完成: ${FILENAME}.gz" | mail -s "备份通知" admin@stardust.app
```

**备份计划**
```
全量备份: 每天凌晨 2:00
增量备份: 每小时
保留周期: 7天本地，30天云端
恢复测试: 每月一次
```

#### 3.2 代码备份
```
Git 仓库: GitHub / GitLab
  - 主分支保护
  - PR 审查机制
  - 定期快照

配置备份:
  - Docker Compose 文件
  - 环境变量
  - Nginx 配置
  - 数据库 Schema
```

---

## 🚀 发布计划时间表

### 第一阶段: MVP 发布 (第1个月)

#### Week 1-2: 开发完成
```
□ 前端: 首页、登录、星盘计算、祈愿
□ 后端: 用户、认证、星盘、祈愿 API
□ 数据库: 表结构、索引
□ 测试: 单元测试、集成测试
```

#### Week 3: 测试环境部署
```
□ 测试环境搭建
□ 自动化测试
□ Bug 修复
□ 性能测试
```

#### Week 4: 生产环境部署
```
□ 生产环境搭建
□ 压力测试
□ 安全检查
□ 监控配置
□ 灰度发布 (10% 用户)
```

### 第二阶段: 功能完善 (第2个月)

#### Week 5-6: 新功能开发
```
□ 月老红线
□ 树洞社区
□ 缘分匹配
□ 实时聊天
```

#### Week 7: 多端发布
```
□ H5 网站正式上线
□ 微信小程序提交审核
□ 抖音小程序提交审核
□ App 打包测试
```

#### Week 8: 运营启动
```
□ 内容生产启动
□ KOC 招募
□ 社群建立
□ 数据监控
```

### 第三阶段: 正式推广 (第3-6个月)

#### Month 3: 增长期
```
□ 全渠道推广
□ 付费投放测试
□ 会员体系上线
□ 数据优化
```

#### Month 4-6: 扩张期
```
□ 扩大投放规模
□ 专家入驻
□ 电商功能
□ 线下活动
□ 品牌合作
```

---

## 📊 发布检查清单

### 发布前检查
```bash
□ 代码审查通过
□ 所有测试通过
□ 文档更新完成
□ 数据库备份
□ 配置文件检查
□ 环境变量检查
□ SSL 证书有效
□ 域名解析正常
□ CDN 配置完成
□ 监控告警配置
□ 回滚方案准备
□ 团队通知到位
```

### 发布中检查
```bash
□ 备份当前版本
□ 逐步部署 (蓝绿部署)
□ 健康检查通过
□ 日志监控正常
□ 关键功能测试
□ 性能指标正常
□ 错误率监控
□ 用户反馈监控
```

### 发布后检查
```bash
□ 全量回归测试
□ 数据一致性检查
□ 性能对比分析
□ 用户反馈收集
□ 问题修复跟进
□ 发布报告总结
```

---

## 🆘 应急预案

### 1. 服务宕机
```
现象: 服务无法访问

处理:
  1. 检查 Docker 容器状态
  2. 查看日志定位问题
  3. 尝试重启服务
  4. 回滚到上一版本
  5. 通知用户

预防:
  - 健康检查
  - 自动重启
  - 负载均衡
```

### 2. 数据库故障
```
现象: 数据库连接失败/数据异常

处理:
  1. 检查数据库状态
  2. 查看错误日志
  3. 从备份恢复
  4. 数据一致性检查

预防:
  - 主从复制
  - 定期备份
  - 监控告警
```

### 3. 性能问题
```
现象: 响应缓慢/超时

处理:
  1. 查看监控指标
  2. 分析慢查询
  3. 扩容或优化
  4. 限流降级

预防:
  - 性能测试
  - 缓存策略
  - 数据库优化
```

### 4. 安全事件
```
现象: 异常访问/数据泄露

处理:
  1. 立即隔离系统
  2. 修复漏洞
  3. 通知用户修改密码
  4. 报告监管部门

预防:
  - 定期安全扫描
  - 代码安全审查
  - 数据加密
  - 访问控制
```

---

## 💰 成本预算

### 首年总成本

#### 开发成本
```
前端开发: ¥40,000 (2人×2月)
后端开发: ¥20,000 (1人×2月)
UI/UX设计: ¥10,000 (1人×1月)
测试: ¥5,000 (0.5人×1月)
小计: ¥75,000
```

#### 基础设施 (首年)
```
云服务器: ¥2,400 (¥200/月 × 12)
数据库: ¥4,800 (¥400/月 × 12)
Redis: ¥1,200 (¥100/月 × 12)
对象存储: ¥600 (¥50/月 × 12)
CDN: ¥240 (¥20/月 × 12)
域名: ¥100
SSL证书: ¥500
小计: ¥9,840
```

#### 运营成本 (首年)
```
内容生产: ¥60,000 (¥5,000/月 × 12)
广告投放: ¥120,000 (¥10,000/月 × 12)
KOL合作: ¥60,000 (¥5,000/月 × 12)
社群运营: ¥24,000 (¥2,000/月 × 12)
小计: ¥264,000
```

#### 其他成本
```
工具软件: ¥5,000
应急储备: ¥20,000
小计: ¥25,000
```

**总计: ¥373,840**

---

## 📈 预期收益

### 第一年
```
用户规模: 20万注册用户
付费用户: 1.6万 (8%)
ARPU: ¥50/月
月收入: ¥80万
年收入: ¥960万

ROI: 2.57:1
```

### 第二年
```
用户规模: 100万注册用户
付费用户: 10万 (10%)
ARPU: ¥60/月
月收入: ¥600万
年收入: ¥7200万

ROI: 10:1
```

---

## 🎯 成功标准

### 短期目标 (3个月)
```
□ 注册用户: 50,000
□ DAU: 5,000
□ 付费转化: 5%
□ 月收入: ¥200,000
□ 用户留存: 40%
```

### 中期目标 (6个月)
```
□ 注册用户: 200,000
□ DAU: 20,000
□ 付费转化: 8%
□ 月收入: ¥1,000,000
□ 用户留存: 45%
```

### 长期目标 (12个月)
```
□ 注册用户: 500,000
□ DAU: 50,000
□ 付费转化: 10%
□ 月收入: ¥3,000,000
□ 用户留存: 50%
□ 品牌知名度: 行业前十
```

---

**文档版本**: v1.0
**最后更新**: 2025-01-09
**状态**: 规划中

*注: 本计划需根据实际情况调整，建议分阶段实施，持续迭代优化。*
